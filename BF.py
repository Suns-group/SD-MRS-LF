    ...
    for spw in Speed_w:
        ...
        for siz in taille:
            for coord in range(3):
                ...
                grid_w = grid_h = user_taille_instances
                ...
                for charge_points in liste_instances:
                    ...
                    # Searching for the best BF solution between vertical and horizontal
                    S_v = [[], float("inf"), float("inf")]
                    S_h = [[], float("inf"), float("inf")]
                    S_0[0] = []
                    S_v[0] = back_and_forth_V(grid_w, grid_h)
                    S_h[0] = back_and_forth_H(grid_w, grid_h)
                    S_v_o = f_objective(S_v, charge_points, takeoff, maximum_energy_3dr_solo,
                                        distance_unit, grid_w, wind_speed,
                                        v0, wind_vector, alpha, beta)
                    S_h_o = f_objective(S_h, charge_points, takeoff, maximum_energy_3dr_solo,
                                        distance_unit, grid_w, wind_speed,
                                        v0, wind_vector, alpha, beta)
                    S_0[0] = S_v[0]
                    if ((S_h_o[2] < S_v_o[2]) or (S_h_o[2] == S_v_o[2] and S_h_o[5] < S_v_o[
                        5])):  # or (nNPC > e[0] and nET < e[1] and P < P_solution_1):
                        S_0[0] = S_h[0]
                    for idx in range(1):
                        # done = False
                        ...
                        if ((S_h_o[2] < S_v_o[2]) or (S_h_o[2] == S_v_o[2] and S_h_o[5] < S_v_o[
                            5])):  # or (nNPC > e[0] and nET < e[1] and P < P_solution_1):
                            S_0[0] = S_h[0]
                        th1 = theta([V_x[coord], V_y[coord]], [1, 0.01])
                        th2 = theta([V_x[coord], V_y[coord]], [0.001, 1])

                        # If the BF does not find a solution, then consider the direction smallest angle with the wind
                        if S_h_o[2] == float('Inf') and S_v_o[2] == float('Inf'):
                            th1 = theta([V_x[coord], V_y[coord]], [1, 0.01])
                            th2 = theta([V_x[coord], V_y[coord]], [0.001, 1])
                            if th1 < th2:
                                S_0[0] = S_v[0]
